...
try {
    ...
    $process = Start-Process -FilePath (Join-Path $testRoot 'Pancake.exe') -ArgumentList '--verify-ui' -WorkingDirectory $testRoot -WindowStyle Hidden -PassThru
    if (-not $process.WaitForExit(60000)) { throw '隔离 UI 验证在 60 秒内未结束。' }
    $result = Join-Path $testRoot 'verification/result.txt'
    ...
    if ($lines[-1] -ne 'UI_VERIFICATION_OK') { $lines | Select-Object -Last 12; throw '隔离 UI 回归检查失败。' }
