...
/// <summary>
/// 所有功能图标的语义名称与随程序分发的 Fluent System Icons 字体版本一一对应。
/// 这里只保存字形码位，界面层负责把它渲染成图标控件。
/// </summary>
public static class FluentGlyphs
{
    ...
    public static string Resolve(string symbol) => symbol switch
    {
        ...
        // 允许直接传入字形码位（图标字体使用私有区），避免调用方混用两种写法时崩溃；
        // 其余无法识别的名称仍然抛错，以便尽早发现拼写错误。
        _ when IsGlyph(symbol) => symbol,
        _ => throw new ArgumentException("未知图标名称", nameof(symbol))
    };

    /// <summary>是否为图标字体里的字形码位（私有使用区）。</summary>
    private static bool IsGlyph(string value) =>
        value.Length > 0 && value[0] is >= '\uE000' and <= '\uF8FF';
}
