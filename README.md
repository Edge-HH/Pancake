...
- 网格布局：磁贴默认吸附到 48 px 网格；网格开启时，自动排列按整格位置和尺寸紧密放置磁贴，空间不足会明确提示。
...
- 自动更新：启动时检查 GitHub Release；便携版 ZIP 下载完成后，确认即可保存项目、自动解压校验、退出覆盖并重新启动，保留项目数据，覆盖失败会尝试恢复旧文件。
- 显示设置：支持深色、浅色和跟随系统主题，以及全屏和窗口模式。
- 自适应布局：窄窗口下自动切换为上下排列。

## 🖥️ 使用方式

程序默认以全屏展示模式启动。查看模式下，底部浮动工具栏按编辑看板、设置、全屏切换排列；编辑模式仍以画笔开头、放弃和保存结尾。
...
- Release 中优先选择 x64 便携版 ZIP 自动更新；`.exe`、`.msix` 和 `.msixbundle` 仍按安装包启动。文件被占用或目录不可写时可能无法更新，可查看保留的备份和日志。
...
- 设置采用 WinUI 侧边栏导航，左侧切换外观、组件和关于，右侧独立滚动；左右面板使用直角连接，背景直接透出 Windows Mica 系统材质并由系统主题生成明暗层级，窗口失焦时仍保留材质色彩。不支持 Mica 时由系统回退到 Desktop Acrylic。
